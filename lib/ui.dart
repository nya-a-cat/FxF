  ...
  Widget build(BuildContext context) => Container(
        decoration: const BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [Color(0xFFE7F9FF), Color(0xFFFBFDFF), Color(0xFFEAF8FF)],
          ),
        ),
...
class _Bubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: const LinearGradient(colors: [Color(0x55FFFFFF), Color(0x2220C6D7)]),
        ),
      );
}
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(22),
              border: Border.all(color: Colors.white),
              boxShadow: const [BoxShadow(color: Color(0x12006A8E), blurRadius: 22, offset: Offset(0, 8))],
            ),
            child: child,
          ),
        ),
...
class FxFHeader extends StatelessWidget {
  const FxFHeader({super.key, this.subtitle});
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(children: [
          Container(
            ...
            decoration: const BoxDecoration(
              shape: BoxShape.circle,
              gradient: LinearGradient(colors: [Color(0xFF8BE8F4), Color(0xFF1EAFC8)]),
            ),
          ...
          Column(
            ...
            children: [
              Text('FxF', style: Theme.of(context).textTheme.headlineMedium),
              if (subtitle != null && subtitle!.trim().isNotEmpty)
                Text(subtitle!, style: const TextStyle(color: FxFColors.muted, fontSize: 10)),
            ],
          ),
          const Spacer(),
          const CharacterAvatar(size: 34),
          const SizedBox(width: 8),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
            decoration: BoxDecoration(color: Colors.white.withValues(alpha: .75), borderRadius: BorderRadius.circular(18)),
            child: const Row(children: [
              CircleAvatar(radius: 4, backgroundColor: FxFColors.positive),
              SizedBox(width: 6),
              Text('LIVE', style: TextStyle(color: FxFColors.primaryDark, fontWeight: FontWeight.w800, fontSize: 11)),
            ]),
          ),
        ]),
      );
...
class CharacterAvatar extends StatelessWidget {
  const CharacterAvatar({super.key, this.size = 42});
  final double size;
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [BoxShadow(color: Color(0x22006A8E), blurRadius: 12)],
        ),
        clipBehavior: Clip.antiAlias,
        child: Image.asset('assets/characters/mascot_avatar.webp', fit: BoxFit.cover),
      );
}

class AnimeArtworkCard extends StatelessWidget {
  const AnimeArtworkCard({
    super.key,
    this.asset = 'assets/characters/mascot_idle.webp',
    this.height = 190,
          ...
          if (snapshot.hasError) {
            return GlassCard(
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Icon(Icons.cloud_off_rounded, color: FxFColors.negative),
                const SizedBox(width: 10),
                Expanded(child: Text('数据源暂不可用\n${snapshot.error}', style: const TextStyle(color: FxFColors.muted))),
              ]),
            );
          }
...
class QuoteTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GlassCard(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Spacer(),
          Text(
            quote.price.toStringAsFixed(quote.price < 100 ? 4 : 2),
            style: const TextStyle(color: FxFColors.ink, fontWeight: FontWeight.w800, fontSize: 20),
          ),
          Text(
            '${quote.changePercent >= 0 ? '+' : ''}${quote.changePercent.toStringAsFixed(2)}%',
            style: TextStyle(
              color: quote.changePercent >= 0 ? FxFColors.positive : FxFColors.negative,
      ...
      child: LineChart(
        LineChartData(
          ...
          gridData: FlGridData(
            ...
            getDrawingHorizontalLine: (_) => const FlLine(color: FxFColors.line, strokeWidth: 1),
          ),
          titlesData: const FlTitlesData(show: false),
          borderData: FlBorderData(show: false),
          lineBarsData: [
            LineChartBarData(
              spots: [for (var i = 0; i < sample.length; i++) FlSpot(i.toDouble(), sample[i].close)],
              isCurved: true,
              barWidth: 2.5,
              color: negative ? FxFColors.negative : FxFColors.primary,
              dotData: const FlDotData(show: false),
              belowBarData: BarAreaData(
                show: true,
                color: (negative ? FxFColors.negative : FxFColors.primary).withValues(alpha: .10),
              ),
            ),
          ],
        ),
...
class MetricBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GlassCard(
        padding: const EdgeInsets.all(13),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: const TextStyle(color: FxFColors.muted, fontSize: 11)),
