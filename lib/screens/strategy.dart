        ...
        children: [
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const Text('历史 K 线 · 本地策略计算', style: TextStyle(color: FxFColors.muted)),
                ...
                child: Column(children: [
                  ...
                  Align(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: const Color(0xFFF4FBFD), borderRadius: BorderRadius.circular(14)),
                      child: Text(
                        'universe: ${symbolController.text.trim().toUpperCase()}\n'
                        'signal: cross(EMA(close,${fast.round()}), EMA(close,${slow.round()}))\n'
                        'execution: close\n'
                        'costs: fee=${fee.toStringAsFixed(1)}bps, slippage=${slippage.toStringAsFixed(1)}bps',
                        style: const TextStyle(fontFamily: 'monospace', color: FxFColors.primaryDark),
                      ),
                    ),
                  ),
                  const SizedBox(height: 12),
              ...
              if (resultFuture != null) ...[
                const SizedBox(height: 12),
                AsyncPane<({BacktestResult result, List<Candle> candles})>(
                  future: resultFuture!,
                  builder: (_, data) => _BacktestResultPanel(data),
                ),
              ],
            ]),
          ),
...
class _SliderParam extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
        SizedBox(width: 120, child: Text('$label ${value.toStringAsFixed(1)}')),
        Expanded(child: Slider(value: value, min: min, max: max, onChanged: onChanged)),
      ]);
}

class _BacktestResultPanel extends StatelessWidget {
  const _BacktestResultPanel(this.data);
  final ({BacktestResult result, List<Candle> candles}) data;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      Row(children: [
        Expanded(child: MetricBox(label: '总收益', value: pct(r.totalReturn), color: r.totalReturn >= 0 ? FxFColors.positive : FxFColors.negative)),
        const SizedBox(width: 8),
        Expanded(child: MetricBox(label: 'CAGR', value: pct(r.cagr))),
        const SizedBox(width: 8),
        Expanded(child: MetricBox(label: '最大回撤', value: pct(r.maxDrawdown), color: FxFColors.negative)),
      ]),
      const SizedBox(height: 8),
      Row(children: [
        Expanded(child: MetricBox(label: 'Sharpe', value: r.sharpe.toStringAsFixed(2))),
        const SizedBox(width: 8),
        Expanded(child: MetricBox(label: 'Sortino', value: r.sortino.toStringAsFixed(2))),
        const SizedBox(width: 8),
        Expanded(child: MetricBox(label: 'Trades', value: '${r.trades}', sub: '胜率 ${(r.winRate * 100).toStringAsFixed(1)}%')),
      ]),
      const SizedBox(height: 8),
      GlassCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('行情区间', style: TextStyle(fontWeight: FontWeight.w800)),
          const SizedBox(height: 8),
          CandleLineChart(candles: data.candles, height: 180),
        ]),
      ),
    ]);
  }
