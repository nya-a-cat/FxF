          ...
          children: [
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const Text('成交记录观察价 · 当前报价 mark-to-market', style: TextStyle(color: FxFColors.muted)),
                const SizedBox(height: 12),
                AsyncPane<({List<ResearchFill> fills, Map<String, MarketQuote> marks})>(
                  future: future,
                  builder: (_, data) {
                    if (data.fills.isEmpty) {
                      return const GlassCard(child: Text('暂无研究成交'));
                    }
                    ...
                    for (final f in data.fills.reversed) {
                      ...
                      positions[f.symbol] = (
                        qty: prev.qty + sign * f.quantity,
                        cost: prev.cost + sign * f.quantity * f.price,
                      );
                    }
                    final rows = positions.entries.where((e) => e.value.qty.abs() > 1e-12).toList();
                    final totalPnl = rows.fold<double>(0, (sum, e) {
                      final mark = data.marks[e.key]?.price;
                      return mark == null ? sum : sum + e.value.qty * mark - e.value.cost;
                    });
                    return Column(children: [
                      MetricBox(
                        label: '未实现 P/L',
                        value: totalPnl.toStringAsFixed(2),
                        color: totalPnl >= 0 ? FxFColors.positive : FxFColors.negative,
                        sub: 'Binance mark',
                      ),
                      ...
                      GlassCard(
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text('最近成交', style: TextStyle(fontWeight: FontWeight.w800)),
                          const SizedBox(height: 8),
                          for (final f in data.fills.take(12))
                            Text(
                              '${f.side} ${f.symbol} ${f.quantity} @ ${f.price.toStringAsFixed(6)}',
      ...
      child: Row(children: [
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              'Qty ${qty.toStringAsFixed(6)} · Avg ${avg.toStringAsFixed(4)}',
              style: const TextStyle(color: FxFColors.muted, fontSize: 11),
            ),
          ]),
        ),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text(mark == null ? 'Mark —' : mark!.toStringAsFixed(4)),
          if (pnl != null)
            Text(
